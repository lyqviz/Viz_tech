...
d3.csv('../../data/nyc_permits.csv', parse)
    ...
    console.log(extentY);
		...
		const scaleX = d3.scaleTime()//.domain(extentX).range([0, w]);
		      .domain([new Date(2017, 3, 27), new Date(2018, 8, 26)])
		      .range([0, w])

		//scaleX(new Date(2017, 3, 27, 3));
		//scaleX(new Date(2018, 8, 26, 15));
		//scaleX.invert(100);
		//scaleX.invert(700);
    ...
    const points = plot2.selectAll('.point')
      ...
      .enter()
      .append('circle')
      .attr('cx', function(d){ return scaleX(d.week)})
      .attr('cy', function(d){ return scaleY(d.permits.length)})
      .attr('r', 5)
